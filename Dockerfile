...
RUN mkdir backend/src && printf 'fn main() {}' > backend/src/main.rs && cargo build --release -p open-gallery-api && rm -rf backend/src
...
RUN touch backend/src/main.rs && cargo build --release -p open-gallery-api
...
WORKDIR /app
COPY --from=api-builder /src/target/release/open-gallery-api /usr/local/bin/open-gallery-api
...
LABEL org.opencontainers.image.title="Open Gallery" \
      org.opencontainers.image.source="https://github.com/Uniseem/open-gallery" \
      org.opencontainers.image.description="Album-first self-hosted gallery with a Rust backend" \
      org.opencontainers.image.licenses="MIT"
EXPOSE 8080
ENTRYPOINT ["docker-entrypoint.sh"]
CMD ["open-gallery-api"]
