...
上传必须指定一个已存在的相簿。应用本身不限制单次选择的图片数量、单张大小或总大小；后台会逐张提交并确认，某张图片失败不会影响此前已经成功入库的图片，失败项可以直接重试。服务仍会校验扩展名、文件签名和完整解码，只接受 PNG、JPG/JPEG 与 WEBP。若通过第三方反向代理或 CDN 访问，还需确保其请求体大小和超时配置不会额外限制上传。
