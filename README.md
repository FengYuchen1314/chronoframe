...
查看器以相册页上的覆盖层打开（链接形如 `/albums/相册ID?photo=图片ID`），不会卸载相册、在背后渲染全站图库或在关闭时重新请求相册。原来的 `/图片ID` 链接仍可用，只查询该图的元数据后定位到所属相册。关闭动画只执行一次 240ms 的位移与缩放，支持浏览器返回、单击关闭、手机下滑关闭和减少动态效果设置。
